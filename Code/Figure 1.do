...
tw line price_carbon dt if sample==1 & country=="GR", sort xlabel(22371 22431 22492 22553 22614 22675 22736 22797, format(%tdMon-CCYY)) xtitle("") ytitle("Carbon price (EUR/ton CO2)",size(*1.2)) lwidth(*1.4) lcolor(emerald*0.8) name(c,replace)  title("C)",position(11) size(*1.4)) text(47 22384 "45€", color(black)) text(76 22717 "75€", color(black)) legend(off) || (line yline xline if !missing(yline), lwidth(*2) lcolor(black))  || (line yline2 xline2 if !missing(yline2), lwidth(*2) lcolor(black)) 
graph export "fig1d.svg", as(svg) width(3000) replace



**Panel C (now is panel B)
cap drop yline xline
...
tw line avg_coallig dt if sample==1 & country=="GR", sort xlabel(22371 22431 22492 22553 22614 22675 22736 22797, format(%tdMon-CCYY)) title("D)",position(11) size(*1.4)) xtitle("") ytitle("Average Coal Generation (MWh)",size(*1.2)) lwidth(*1.2) lcolor(maroon*0.75) name(cos,replace) legend(off)  text(2910 22384 "2817", color(black)) text(4110 22717 "4005", color(black)) || (line yline xline if !missing(yline), lwidth(*2) lcolor(black))  || (line yline2 xline2 if !missing(yline2), lwidth(*2) lcolor(black)) 
graph export "fig1b.pdf", as(pdf) width(3000) replace
